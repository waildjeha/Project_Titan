...
// Get the position on Earth's surface pointing toward Titan
    ...
    goodGuesses = findLocalMinima(pointedAtTitan, -60, 60, -60, 60, -60, 60, 1);

    }

private List<Probe> findLocalMinima(Vector position, double minVx, double maxVx,
                                    double minVy, double maxVy, double minVz, double maxVz,
                                    double stepSize) {
        List<Probe> goodGuesses = new ArrayList<>();
...
//                      System.out.println(closestDistanceTmp + " " + rk4Probe.getClosestDistTime().toString());
                        ...
                        goodGuesses.add(initialProbe);
                        }
                        else if(stepSize>1) {
                            if (closestDistanceTmp < (T0_DIST_TO_TITAN / (Math.pow(stepSize, 2)))) {
                            System.out.println("--------------------------------------\n" +
                                    "NEW GUESS");
                            System.out.println(closestDistanceTime);
                            System.out.println(initialProbe);
