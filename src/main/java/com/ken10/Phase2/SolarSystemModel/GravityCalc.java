...
public class GravityCalc {
     ...
     */
    public static ArrayList<CelestialBodies> computeDerivatives(LocalDateTime time, ArrayList<CelestialBodies> bodies) {
        ...
        for (int i = 0; i < bodies.size(); i++) {
            ...
            if (original instanceof PlanetModel) {
                ...
                derivativeBody = new Titan(original.getName(), positionDerivative, acceleration, original.getMass());
            }
            else derivativeBody = new Earth(original.getName(), positionDerivative, acceleration, original.getMass());
